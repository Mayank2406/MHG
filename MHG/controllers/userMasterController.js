...
// Since in Debit api and credit api we need to check if order_id already existed in
// user_history then transaction couldn't be done else if order_id is new then transaction
// transaction could be done.

// Creating function to check if order_id exists or not

var f = false;
var checkOrderId = (order_id, callback) => {
    UserHistory.findOne({ order_id: order_id })
        .then(data => { f = true; callback(data) })
